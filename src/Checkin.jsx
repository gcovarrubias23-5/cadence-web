...
export function Checkin({ marks, goal, lastMove, onPulse, onPatch, platesAte, plateGoal, waterDrank, waterGoal }) {
  return (
    <>
      <section className="hero">
        <h1>Weekly check-in.</h1>
        <p>Look at the week, say how the food sat, and next week’s plates move from that.</p>
      </section>
      ...
      <section className="card">
        ...
        <ProgressBars ate={Math.min(6, Math.round((platesAte / Math.max(plateGoal, 1)) * 6))} drinks={Math.min(WATER_GOAL, Math.round((waterDrank / Math.max(waterGoal, 1)) * WATER_GOAL))} />
        <p className="note">This is the week as a whole, not one day.</p>
      </section>
      ...
      <section className="card">
        ...
        <p className="note" style={{ marginTop: 0 }}>These follow the check-in. You can still type a number if you need to.</p>
